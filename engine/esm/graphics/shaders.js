...
ThickLineShader.vertLoc = 0;
ThickLineShader.prevVertLoc = 0;
...
ThickLineShader.positionOffset = 12;
ThickLineShader.nextOffset = 24;
ThickLineShader.colorOffset = 36;
ThickLineShader.timeOffset = 52;
ThickLineShader.thicknessOffset = 60;
ThickLineShader.orientationOffset = 64;

ThickLineShader.init = function (renderContext) {
    var gl = renderContext.gl;
    ...
    ThickLineShader._frag = gl.createShader(WEBGL.FRAGMENT_SHADER);
    gl.shaderSource(ThickLineShader._frag, fragShaderText);
    gl.compileShader(ThickLineShader._frag);
    if (!gl.getShaderParameter(ThickLineShader._frag, WEBGL.COMPILE_STATUS)) {
        console.error('ThickLineShader fragment shader: ' + gl.getShaderInfoLog(ThickLineShader._frag));
    }

    ThickLineShader._vert = gl.createShader(WEBGL.VERTEX_SHADER);
    gl.shaderSource(ThickLineShader._vert, vertexShaderText);
    gl.compileShader(ThickLineShader._vert);
    if (!gl.getShaderParameter(ThickLineShader._vert, WEBGL.COMPILE_STATUS)) {
        console.error('ThickLineShader vertex shader: ' + gl.getShaderInfoLog(ThickLineShader._vert));
    }

    ThickLineShader._prog = gl.createProgram();
    gl.attachShader(ThickLineShader._prog, ThickLineShader._vert);
    gl.attachShader(ThickLineShader._prog, ThickLineShader._frag);
    gl.linkProgram(ThickLineShader._prog);
    if (!gl.getProgramParameter(ThickLineShader._prog, WEBGL.LINK_STATUS)) {
        console.error('ThickLineShader link: ' + gl.getProgramInfoLog(ThickLineShader._prog));
    }
    gl.useProgram(ThickLineShader._prog);

    ThickLineShader.prevVertLoc = gl.getAttribLocation(ThickLineShader._prog, 'aPreviousPosition');
    ThickLineShader.vertLoc = gl.getAttribLocation(ThickLineShader._prog, 'aVertexPosition');
    ThickLineShader.nextVertLoc = gl.getAttribLocation(ThickLineShader._prog, 'aNextPosition');
    ThickLineShader.colorLoc = gl.getAttribLocation(ThickLineShader._prog, 'aVertexColor');
    ThickLineShader.timeLoc = gl.getAttribLocation(ThickLineShader._prog, 'aTime');
    ThickLineShader.thicknessLoc = gl.getAttribLocation(ThickLineShader._prog, 'aThickness');
    ThickLineShader.orientationLoc = gl.getAttribLocation(ThickLineShader._prog, 'aOrientation');

    ThickLineShader.lineColorLoc = gl.getUniformLocation(ThickLineShader._prog, 'lineColor');
    ThickLineShader.projMatLoc = gl.getUniformLocation(ThickLineShader._prog, 'uPMatrix');
    ThickLineShader.mvMatLoc = gl.getUniformLocation(ThickLineShader._prog, 'uMVMatrix');
    ThickLineShader.jNowLoc = gl.getUniformLocation(ThickLineShader._prog, 'jNow');
    ThickLineShader.decayLoc = gl.getUniformLocation(ThickLineShader._prog, 'decay');
    ThickLineShader.widthLoc = gl.getUniformLocation(ThickLineShader._prog, 'viewportWidth');
    ThickLineShader.heightLoc = gl.getUniformLocation(ThickLineShader._prog, 'viewportHeight');

    gl.enable(WEBGL.BLEND);
    gl.blendFunc(WEBGL.SRC_ALPHA, WEBGL.ONE_MINUS_SRC_ALPHA);
    ThickLineShader.initialized = true;
};

ThickLineShader.use = function (renderContext, vertex, lineColor, zBuffer, jNow, decay) {
    var gl = renderContext.gl;
    if (gl != null) {
        if (!ThickLineShader.initialized) {
            ThickLineShader.init(renderContext);
        }
        gl.useProgram(ThickLineShader._prog);
        var mvMat = Matrix3d.multiplyMatrix(renderContext.get_world(), renderContext.get_view());
        gl.uniformMatrix4fv(ThickLineShader.mvMatLoc, false, mvMat.floatArray());
        gl.uniformMatrix4fv(ThickLineShader.projMatLoc, false, renderContext.get_projection().floatArray());
        gl.uniform4f(ThickLineShader.lineColorLoc, lineColor.r / 255, lineColor.g / 255, lineColor.b / 255, 1);
        gl.uniform1f(ThickLineShader.jNowLoc, jNow);
        gl.uniform1f(ThickLineShader.decayLoc, decay);
        gl.uniform1f(ThickLineShader.widthLoc, renderContext.width);
        gl.uniform1f(ThickLineShader.heightLoc, renderContext.height);
        if (zBuffer) {
            gl.enable(WEBGL.DEPTH_TEST);
        } else {
            gl.disable(WEBGL.DEPTH_TEST);
        ...
        gl.bindBuffer(WEBGL.ARRAY_BUFFER, vertex);
        gl.bindBuffer(WEBGL.ELEMENT_ARRAY_BUFFER, null);
        gl.enableVertexAttribArray(ThickLineShader.prevVertLoc);
        gl.enableVertexAttribArray(ThickLineShader.vertLoc);
        gl.enableVertexAttribArray(ThickLineShader.nextVertLoc);
        gl.enableVertexAttribArray(ThickLineShader.colorLoc);
        gl.enableVertexAttribArray(ThickLineShader.timeLoc);
        gl.enableVertexAttribArray(ThickLineShader.thicknessLoc);
        gl.enableVertexAttribArray(ThickLineShader.orientationLoc);
        gl.vertexAttribPointer(ThickLineShader.prevVertLoc, 3, WEBGL.FLOAT, false, ThickLineShader.stride, ThickLineShader.prevOffset);
        gl.vertexAttribPointer(ThickLineShader.vertLoc, 3, WEBGL.FLOAT, false, ThickLineShader.stride, ThickLineShader.positionOffset);
        gl.vertexAttribPointer(ThickLineShader.nextVertLoc, 3, WEBGL.FLOAT, false, ThickLineShader.stride, ThickLineShader.nextOffset);
        gl.vertexAttribPointer(ThickLineShader.colorLoc, 4, WEBGL.FLOAT, false, ThickLineShader.stride, ThickLineShader.colorOffset);
        gl.vertexAttribPointer(ThickLineShader.timeLoc, 2, WEBGL.FLOAT, false, ThickLineShader.stride, ThickLineShader.timeOffset);
        gl.vertexAttribPointer(ThickLineShader.thicknessLoc, 1, WEBGL.FLOAT, false, ThickLineShader.stride, ThickLineShader.thicknessOffset);
        gl.vertexAttribPointer(ThickLineShader.orientationLoc, 1, WEBGL.FLOAT, false, ThickLineShader.stride, ThickLineShader.orientationOffset);
        gl.enable(WEBGL.BLEND);
        gl.blendFunc(WEBGL.SRC_ALPHA, WEBGL.ONE_MINUS_SRC_ALPHA);
    }
};
...
ThickLineShader.cleanup = function (renderContext) {
    var gl = renderContext.gl;
    if (gl != null && ThickLineShader.initialized) {
        gl.disableVertexAttribArray(ThickLineShader.prevVertLoc);
        gl.disableVertexAttribArray(ThickLineShader.vertLoc);
        gl.disableVertexAttribArray(ThickLineShader.nextVertLoc);
        gl.disableVertexAttribArray(ThickLineShader.colorLoc);
        gl.disableVertexAttribArray(ThickLineShader.timeLoc);
        gl.disableVertexAttribArray(ThickLineShader.thicknessLoc);
        gl.disableVertexAttribArray(ThickLineShader.orientationLoc);
    }
};

var ThickLineShader$ = {};

registerType("ThickLineShader", [ThickLineShader, ThickLineShader$, null]);
